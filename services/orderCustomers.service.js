...
class OrderCustomerService {
    ...
    createOrderCustomer = async (customerId, orderItems) => {
        let totalAmount = 0;
        const createdOrderCustomer = await this.orderCustomerRepository.createOrderCustomer();
        ...
        for (const orderItem of orderItems) {
            ...
            await this.itemOrderCustomerRepository.createItemOrderCustomer(itemOrderCustomerData);
        }
    ...
    completeOrder = async (id) => {
        // 주문 정보 가져오기
        const orderCustomer = await this.orderCustomerRepository.getOrderCustomerById(id);
        console.log(orderCustomer);

        // 이미 주문이 완료된 경우 에러 발생
        if (orderCustomer.state === true) {
            throw new Error('이미 완료처리된 주문입니다');
        }
        ...
        try {
            ...
            const orderItems = await this.itemOrderCustomerRepository.getItemsByOrderCustomerId(id);
    ...
    cancelOrder = async (id) => {
        ...
        try {
            ...
            await this.itemOrderCustomerRepository.deleteItemOrderCustomersByOrderCustomerId(id, t);
